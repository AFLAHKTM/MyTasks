...
import { getStatuses, saveStatuses, getTasks, updateTask, getPriorities, savePriorities, getWorkspaceConfig, saveWorkspaceConfig } from '../lib/data';
import { Trash2, GripVertical } from 'lucide-react';

export default function Settings() {
    const [statuses, setStatuses] = useState([]);
    const [priorities, setPriorities] = useState([]);
    const [config, setConfig] = useState({ enable_ai_assistant: true, auto_archive_tasks: false, sync_system_clock: true });
        ...
        <>
            ...
            <div style={{ marginBottom: '2.5rem' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: 'var(--text-primary)', fontSize: '0.875rem', cursor: 'pointer' }}>
                        <input
                            type="checkbox"
                            checked={config.enable_ai_assistant}
                            onChange={e => saveWorkspaceConfig({ ...config, enable_ai_assistant: e.target.checked })}
                            style={{ width: '16px', height: '16px' }}
                        /> Enable AI Assistant Module
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: 'var(--text-primary)', fontSize: '0.875rem', cursor: 'pointer' }}>
                        <input
                            type="checkbox"
                            checked={config.auto_archive_tasks}
                            onChange={e => saveWorkspaceConfig({ ...config, auto_archive_tasks: e.target.checked })}
                            style={{ width: '16px', height: '16px' }}
                        /> Auto-archive Done Tasks (Past 30 Days)
                    </label>
                    <label style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: 'var(--text-primary)', fontSize: '0.875rem', cursor: 'pointer' }}>
                        <input
                            type="checkbox"
                            checked={config.sync_system_clock}
                            onChange={e => saveWorkspaceConfig({ ...config, sync_system_clock: e.target.checked })}
                            style={{ width: '16px', height: '16px' }}
                        /> Sync Local System Clock
                    </label>
                </div>
            </div>
