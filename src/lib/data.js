...
const syncFromSupabase = async () => {
  try {
    ...
    if (tasksRes.error || statusesRes.error || prioritiesRes.error) {
      ...
      // If we are in Electron and failing to fetch, it might be a connectivity issue.
      // Don't overwrite local storage if we can't confirm remote is actually empty.
      return; 
    }

    console.log('Successfully fetched from Supabase:', { 
        tasksCount: tasksRes.data?.length,
        notes: notesRes.data ? 'Found' : 'Not found'
    });
